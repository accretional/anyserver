...
do_test() {
    ...
    if lsof -ti ":$TEST_PORT" -sTCP:LISTEN 2>/dev/null; then
        echo "killing existing process on port $TEST_PORT"
        lsof -ti ":$TEST_PORT" -sTCP:LISTEN | xargs kill -9 2>/dev/null || true
    else
    ...
    check_status "http://localhost:$TEST_PORT/static/base.css" "200" "GET /static/base.css"
    check_status "http://localhost:$TEST_PORT/static/docs.css" "200" "GET /static/docs.css"
    check_status "http://localhost:$TEST_PORT/static/app.css" "200" "GET /static/app.css"
